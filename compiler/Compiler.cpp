...
#endif
  int compile_res =
      driver.ExecuteCompilation(*webc_compilation, failingCommands);
  if (compile_res < 0) {
...
int build(std::string *buf, const char *outputPath,
          const std::set<ArgsParser::Options> &opts) {
  if (analysis(buf) == ROK) {
...
const vector<string> *getOpenSSLLibDir() {
  ...
  try {
    Glib::spawn_sync("", argv, Glib::SPAWN_DEFAULT, Glib::SlotSpawnChildSetup(),
                     stdoutput, stderroutput);
    if (stderroutput->empty()) {
      boost::trim_right(*stdoutput);
      auto v = new vector<string>();
      boost::split(*v, *stdoutput, boost::is_any_of(" "),
